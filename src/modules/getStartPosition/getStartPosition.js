...
export const requestGeolocation = (onSuccess, onError) => {
    if (!navigator.geolocation) {
        ...
        (error) => {
            let err;
            switch(error.code) {
                case 1: err = new Error("Пользователь отказал в запросе геолокации."); break;
                case 2: err = new Error("Информация о местоположении недоступна."); break;
                case 3: err = new Error("Превышено время ожидания запроса геолокации."); break;
                default: err = new Error("Произошла неизвестная ошибка при получении геолокации.");
            }
            if (onError) onError(err);
            else throw err;
        },
